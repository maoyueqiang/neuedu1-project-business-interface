...
public class AdminAuthorityInterceptor implements HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws UnsupportedEncodingException {
        ...
        if(userInfo.getRole()!= Const.RoleEnum.ROLE_ADMIN.getCode()){
            ...
            try {
                PrintWriter writer = response.getWriter();
                ServerResponse serverResponse = ServerResponse.createServerResponseByFail(Const.ResponseCodeEnum.NO_PRIVILEGE.getCode(),Const.ResponseCodeEnum.NO_PRIVILEGE.getDesc());
                String json = serverResponse.toString();
                writer.write(json);
                writer.flush();
                writer.close();
